# Hey, I'm Hropiberhtaz 👋

![Student](https://img.shields.io/badge/Student-E6F1FB?style=flat-square&color=185FA5&labelColor=E6F1FB)
![Open Source](https://img.shields.io/badge/Open%20Source-E1F5EE?style=flat-square&color=0F6E56&labelColor=E1F5EE)
![Creative Builder](https://img.shields.io/badge/Creative%20Builder-EEEDFE?style=flat-square&color=534AB7&labelColor=EEEDFE)

> Student · Open Source Contributor · Creative Builder

---

## 🙋 About Me

I'm a student passionate about building things on the web — from clean websites to color palettes and wallpapers.  
I believe in learning by doing and contributing to open source along the way.

Currently focused on sharpening my dev skills, shipping personal projects, and growing in public.

---

## 🛠️ What I Build

| 🎨 Color Palettes | 🌐 Websites | 🖼️ Wallpapers & SVGs |
|---|---|---|
| Made an UI color systems for designers & devs (Side Note:  I made it for myself lol) | Clean, minimal and some interactive webpages built from scratch | Recolored or just lazy to go to other sites so those wallpapers are here lol & white SVGs for my discord and stuff |

---

## 📌 Featured Projects

### 🌐 [Hropiberhtaz.github.io](https://github.com/Hropiberhtaz/Hropiberhtaz.github.io)
> Working to make something (I guess) — built with CSS

### 🖼️ [My-wallpapers-and-svgs](https://github.com/Hropiberhtaz/My-wallpapers-and-svgs)
> White SVGs and some cool wallpapers — built with HTML

---

## 📌 Currently

- 📚 Learning and leveling up every day
- 🔧 Building side projects and contributing to open source
- 🤝 Looking to collaborate on creative & frontend projects
- 🌱 Growing in public — one commit at a time

---


*Thanks for stopping by! Feel free to check out my repos and leave a ⭐ if something catches your eye.*
